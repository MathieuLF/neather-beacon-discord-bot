...
const cacheTtlMs = () => readPositiveInteger(process.env.BOT_POKEAPI_CACHE_TTL_DAYS, 30) * 24 * 60 * 60 * 1000;
const maxAssetBytes = () => readPositiveInteger(process.env.BOT_POKEAPI_MAX_ASSET_BYTES, 5 * 1024 * 1024);
...
const readCache = (endpoint) => {
  ...
  try {
    ...
    endpointMemoryCache.set(endpoint, {
    ...
    const request = https.get(
      ...
      (response) => {
        ...
        if (Number.isFinite(declaredLength) && declaredLength > maxAssetBytes()) {
          response.resume();
          settle(new Error(`asset exceeds ${maxAssetBytes()} bytes`));
          return;
        }

        if (response.statusCode < 200 || response.statusCode >= 300) {
          response.resume();
          settle(new Error(`asset download returned HTTP ${response.statusCode}`));
          return;
        }
        ...
        response.on('data', (chunk) => {
          ...
          if (receivedBytes > maxAssetBytes()) {
            request.destroy(new Error(`asset exceeds ${maxAssetBytes()} bytes`));
            return;
          }
