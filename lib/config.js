...
const peerStateDir = process.env.BOT_PEER_STATE_DIR || path.join(rootDir, 'runtime', 'peer-state');
...
const paths = {
  ...
  peerStateDir,
  museStatePath: path.join(peerStateDir, 'muse-state.json'),
};
...
const validateSemantics = (plan) => {
  ...
  if (!roleNames.has(plan.adminRoleName)) throw new Error('adminRoleName references an unknown role');
  ...
  for (const section of plan.sections) {
    ...
    for (const channel of section.channels) {
      if (Boolean(channel.private) === Boolean(channel.everyonePreset)) {
        throw new Error(`Channel ${channel.name} must declare exactly one visibility mode: private or everyonePreset`);
      }
...
const loadServerPlan = () => validateServerPlan(JSON.parse(fs.readFileSync(configPath, 'utf8')));
