...
const buildDailySummaryMessage = (settings, dateKey, availability) => {
  const summaryUrl = getSummaryUrl(settings, dateKey);
  const label = formatDateLabel(dateKey);
  const description = [
    availability?.ok && availability?.indexOk
      ? 'Le récap de la veille est disponible : ouvre-le pour revoir les stats et les moments de la journée.'
      : availability?.ok
        ? 'La page du récap est accessible, mais ses données n’ont pas pu être confirmées.'
        : 'Le récap n’est pas confirmé disponible pour le moment. Tu peux réessayer plus tard ou ouvrir le lien.',
    '',
    summaryUrl,
  ].filter(Boolean).join('\n');

  const embed = new EmbedBuilder()
    .setColor(availability?.ok && availability?.indexOk ? 0x2f8f83 : 0xe8a23a)
