...
const planPalworldPlayerAnnouncements = (snapshot, previousState) => {
  ...
  if (!shouldRebaseline) {
    const joined = Object.entries(currentPlayers)
      .filter(([key]) => !state.players[key])
      .map(([, player]) => player);
    const left = Object.entries(state.players)
      .filter(([key]) => !currentPlayers[key])
      .map(([, player]) => player);

    if (joined.length) {
      messages.push(playerChangeAnnouncement('join', joined, snapshot.players.length));
    }

    if (left.length) {
      messages.push(playerChangeAnnouncement('leave', left, snapshot.players.length));
    }
  }
