...
const buildDesiredOverwrites = (roleMap, channelDef) => {
  ...
  if (channelDef.private) {
    mergeDesiredOverwrite(overwriteMap, everyone.id, 0n, allManagedBits);
...
const auditRoles = (guild, report, registry) => {
  for (const roleDef of plan.roles) {
    ...
    const desiredPermissions = toBits(roleDef.permissions);
    if (result.value.permissions.bitfield !== desiredPermissions) {
      rememberFix(report, `les permissions du rôle ${roleDef.name} dérivent du plan strict`);
    }
