...
const uptimeKumaUnavailableAnnouncement = ({ statusPageUrl, error, checkedAt }) => [
  '**⚠️ Uptime Kuma indisponible**',
  '',
  "Alpha n'arrive pas à lire la page de statut Palworld. Aucun message public n'a été envoyé dans le salon Palworld.",
  `- **Page statut** : ${statusPageUrl}`,
  `- **Erreur** : ${error.message}`,
  `- **Détecté** : ${checkedAt}`,
].join('\n');

const uptimeKumaRecoveredAnnouncement = (snapshot) => [
  '**✅ Uptime Kuma est revenu**',
  '',
  'Alpha lit de nouveau la page de statut Palworld.',
  `- **Page statut** : ${snapshot.pageUrl}`,
  `- **Statut Palworld lu** : ${snapshot.overallStatus}`,
...
const planUptimeKumaAnnouncements = (snapshot, previousState) => {
  ...
  if (state.uptimeKumaReachable === false) {
    logMessages.push(uptimeKumaRecoveredAnnouncement(snapshot));
  }
...
const planUptimeKumaFetchFailure = ({
  error,
  previousState,
  statusPageUrl,
  checkedAt = new Date().toISOString(),
  ...
  const state = normalizeState(previousState);
  const shouldAnnounce = state.uptimeKumaReachable !== false;

  return {
    logMessages: shouldAnnounce
      ? [uptimeKumaUnavailableAnnouncement({ statusPageUrl, error, checkedAt })]
      : [],
    state: normalizeState({
      ...state,
      updatedAt: checkedAt,
      uptimeKumaReachable: false,
