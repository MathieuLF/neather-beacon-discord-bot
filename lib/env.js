...
const validateBotEnvironment = (env = process.env, { requireDiscord = true } = {}) => {
  const errors = [];
  const dailySummaryChannelIds = parseList(env.GAYLEMON_DAILY_SUMMARY_CHANNEL_IDS);
  ...
  validateSnowflakeList(dailySummaryChannelIds, 'GAYLEMON_DAILY_SUMMARY_CHANNEL_IDS', errors);
  ...
  const normalized = {
    ...
    GAYLEMON_DAILY_SUMMARY_CHANNEL_IDS: dailySummaryChannelIds,
