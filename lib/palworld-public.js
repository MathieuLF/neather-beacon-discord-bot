...
const freshnessIssuesFromAvailability = (payload) => {
  const issues = [];
...
const freshnessIssuesFromMetrics = (payload, nowMs, maxAgeSeconds) => {
  ...
  const updatedAtMs = parseDateTime(payload?.updatedAt || payload?.provenance?.observedAt);
  if (updatedAtMs && Number.isFinite(maxAgeSeconds) && nowMs - updatedAtMs > maxAgeSeconds * 1000) {
    issues.push('metrics-age-exceeded');
  }

  return issues;
};

const normalizePublicAvailability = (payload, { nowMs = Date.now() } = {}) => {
  ...
  return {
    ...
    fresh: freshnessIssuesFromAvailability(safePayload, nowMs).length === 0,
    freshnessIssues: freshnessIssuesFromAvailability(safePayload, nowMs),
  };
};
...
const mergePublicStatus = ({ availability, metrics, checkedAt }) => {
  ...
  return {
    ...
    updatedAt: metrics?.updatedAt || availability?.updatedAt || checkedAt,
...
const createPalworldPublicClient = ({
  ...
  const fetchStatus = async () => {
    ...
    const availability = availabilityResult.status === 'fulfilled'
      ? normalizePublicAvailability(availabilityResult.value, { nowMs: now() })
      : null;
